...
# masterVolumeNode = sampleDataLogic.downloadMRBrainTumor1() # 256, 256, 112
#print(masterVolumeNode)

# Create a new volume node that is 200x200x112 and is filled with zeros. In the center, put ones. Overwrite the masterVolumeNode with this new volume node.
image = vtk.vtkImageData()
image.SetOrigin([0,0,0])
image.SetSpacing([1,1,1])
image.SetDimensions([200,200,112])
image.SetAndObserveImageData(vtk.vtkImageData())
image.GetImageData().AllocateScalars(vtk.VTK_DOUBLE, 1)
image.GetImageData().FillComponent(0, 0)
for i in range(50,150):
  for j in range(50,150):
    for k in range(30,112):
      image.GetImageData().SetScalarComponentFromDouble(i,j,k,0,1)

masterVolumeNode = slicer.vtkMRMLScalarVolumeNode()
slicer.mrmlScene.AddNode(masterVolumeNode)
masterVolumeNode.SetAndObserveImageData(image.GetImageData())
masterVolumeNode.CreateDefaultDisplayNodes() # only needed for display
